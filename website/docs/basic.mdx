...
The simplest way to use Styli is to set some Atomic Props directly on the Styli component. The Atomic Props `text2XL`, `fontBold`, `green500` will produce the following style:
...
> The `f` in `text2XL` is the abbreviation of fontSize. For more conciseness and development efficiency, we will simplify the high-frequency Atomic Props such as `margin`, `padding`, `width`, `heigth`, `fontSize`, `color` to one character.
