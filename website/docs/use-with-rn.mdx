...
```bash
npm i @styli/core @styli/react-native @styli/preset-default
```

Besides `@styli/core`, we also need React Native binding package `@styli/react-native`.
...
const ProfileCard = () => (
  <View centerY p-10 w-260 bgWhite rounded-10 shadow>
    <Imagecircle-48 source={{ uri: '/img/jobs.jpg' }} />
    <View ml-10>
      <View f-20 fontBold>
        Steve Jobs
      </View>
