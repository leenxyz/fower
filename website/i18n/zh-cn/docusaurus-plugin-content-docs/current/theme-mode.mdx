...
Styli comes with built-in support for managing theme mode. We call it **theme mode**, not **color mode**.
...
By default, `dark mode` is built-in support in Styli, you can use `--dark` postfix with Atomic Props directly. You can customize theme mode by config theme 'modes' option:
