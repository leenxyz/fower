 ...
 */
const digitReg = /^([mp][xytrbl]?|space[xy]?|space|top|right|bottom|left|[wh]|square|circle|min[hw]|max[hw]|opacity|text|zindex|leading|fontWeight|rounded([tlrb]|t[lr]|b[lr])?)-?-?\d+[a-z]*?$/i
...
export function digitPreprocessor(atom: Atom, styli: any): Atom {
  ...
  atom.key = key
  atom.propValue = isSpace ? spacing[value] : value
